...
%token PRINT_SUCCESS
%token EXPAND_DEFINITIONS
%token INTERACTIVE_MODE
%token PRODUCE_PROOFS
%token PRODUCE_UNSAT_CORES
%token PRODUCE_MODELS
%token PRODUCE_ASSIGNMENTS
%token REGULAR_OUTPUT_CHANNEL
%token DIAGNOSTIC_OUTPUT_CHANNEL
%token RANDOM_SEED
%token VERBOSITY

%token ERROR_BEHAVIOR
%token NAME
%token AUTHORS
%token VERSION
%token STATUS
%token REASON_UNKNOWN
%token ALL_STATISTICS
